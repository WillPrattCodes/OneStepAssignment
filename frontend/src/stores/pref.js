...
export const usePrefStore = defineStore('pref', {
  ...
  actions: {
    ...
    async toggleDeviceVisibility(deviceId) {
      ...
      if (isHidden) {
        ...
        this.preferences.hiddenDevices = this.preferences.hiddenDevices.filter(
          (id) => id !== deviceId,
        )
      } else {
  ...
  getters: {
    ...
    isDeviceHidden: (state) => (deviceId) =>
      Array.isArray(state.preferences.hiddenDevices) &&
      state.preferences.hiddenDevices.includes(deviceId),
  },
})
